	...
	public void findOrdersByDateRange() {
		...
		try {
			...
			if( queryOrderResultList == null) {
				Messages.addGlobalWarn("Unknown orderdate \"{0}\". We found 0 results", queryOrderId);
			} else {
				Messages.addGlobalInfo("There are {0} orders from {1} to {2}", queryOrderResultList.size(), queryStartDate,queryEndDate);
			}			
		} catch (Exception e) {
			Messages.addGlobalError("Unable to perform search.");
		}		
	...
	public void findCustomer() {
			...
			try {
				...
				if( queryCustomerSingleResult == null) {
					Messages.addGlobalWarn("Unknown Value \"{0}\". We found 0 results");
				} else {
					Messages.addGlobalInfo("we found one result");
				}			
			} catch (Exception e) {
				Messages.addGlobalError("Unable to perform search.");
			 } 		
	...
	public void findOrdersByCustomer() {
		// TODO: Complete the code for this method		
			...
			try {
				...
				if( queryOrderResultList == null) {
					Messages.addGlobalWarn("Unknown customerId \"{0}\". We found 0 results");
				} else {
					Messages.addGlobalInfo("we found one result");
				}			
			} catch (Exception e) {
